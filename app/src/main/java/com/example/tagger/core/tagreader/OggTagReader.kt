...
private const val TAG = "OggTagReader"
 ...
 */
class OggTagReader : AudioTagReader {
    ...
    override fun read(file: File, uri: Uri): AudioMetadata? {
        ...
        return if (mmrResult != null && jatResult != null) {
            // 两者都成功，合并结果
            Log.d(TAG, "OGG merged MMR + JAudioTagger: ${file.name}")
            mergeTagFields(mmrResult, jatResult)
        } else if (jatResult != null) {
            // 只有 JAudioTagger 成功
            Log.d(TAG, "OGG read via JAudioTagger: ${file.name}")
            jatResult
        } else if (mmrResult != null) {
            // 只有 MMR 成功
            Log.d(TAG, "OGG read via MediaMetadataRetriever: ${file.name}")
            mmrResult
        } else {
            Log.w(TAG, "OGG read failed for: ${file.name}")
            null
        }
    }
